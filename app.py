...
st.caption(
    "Finnhub para perfil y peers; Alpha Vantage para estados financieros históricos; "
    "Yahoo Finance solo para precios con caché."
)
...
# -------------------------
# SQLite básica (perfiles y peers)
# -------------------------
def get_conn():
...
def cache_profile(symbol, payload):
    ...
    conn.execute(
        ...
        (symbol, name, country, currency, exchange, finnhub_industry, ipo, raw_json, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            symbol,
...
def load_profile(symbol):
    ...
    row = conn.execute(
        """
        SELECT name, country, currency, exchange,
               finnhub_industry, ipo, raw_json
...
def cache_peers(symbol, peers):
    ...
    for p in peers:
        conn.execute(
            """
            INSERT OR REPLACE INTO company_peers
            (symbol, peer, updated_at)
            VALUES (?, ?, ?)
            """,
            (symbol, p, now),
        )
...
def load_peers(symbol):
    conn = get_conn()
    rows = conn.execute(
        "SELECT peer FROM company_peers WHERE symbol=?",
        (symbol,),
    ).fetchall()
    conn.close()
...
def get_peers(symbol, token):
    ...
    # Limitamos a 4 peers para reducir llamadas
...
# -------------------------
# Yahoo Finance: solo precios + info básica (caché)
# -------------------------
...
# -------------------------
# Alpha Vantage: estados financieros (caché)
# -------------------------
...
def alphav_financials(symbol, api_key):
    """
    Obtiene income, balance y cash flow con Alpha Vantage y
    construye las series necesarias para PER, ROIC, margen EBITDA y FCF.
    """
    ...
    def to_series_annual(reports, key):
        vals = {}
        for r in reports:
            if "fiscalDateEnding" in r and key in r and r[key] not in (None, ""):
                try:
    ...
    # En algunos casos Alpha Vantage no devuelve 'eps' en income; podrías
    # complementarlo con el endpoint EARNINGS, pero por simplicidad probamos aquí:
    eps = to_series_annual(inc_ann, "eps")
    ...
    base_fcf = (
        float(fcf.tail(2).mean())
        if len(fcf) >= 2
        else (float(fcf.iloc[-1]) if len(fcf) else np.nan)
    )
...
# DCF y trayectoria
# -------------------------
def dcf_intrinsic_from_base(
    base_fcf, growth_rate, shares, cash, debt, wacc=0.075, term_growth=0.03
):
    if (
        not np.isfinite(base_fcf)
        or base_fcf <= 0
...
def intrinsic_path(base_fcf, growth_rate, shares, cash, debt, years=5):
    ...
    for y in range(years + 1):
        vals[y] = dcf_intrinsic_from_base(
            curr_fcf,
            max(growth_rate * (0.95 ** y), 0.03),
            shares,
            cash,
            curr_debt,
        )
...
# -------------------------
# Tabla métricas desde 2020
# -------------------------
def metrics_table_from_2020(fin):
    revenue = fin["revenue"]
    ebitda = fin["ebitda"]
    ...
    for year in range(2020, current_year + 1):
        row = {
            "Year": year,
            "ROIC_%": np.nan,
            "EBITDA_margin_%": np.nan,
            "EPS": np.nan,
            "Shares": np.nan,  # no usamos aquí porque Alpha no da acciones
            "FCF": np.nan,
        }
        ...
        if (
            ...
            and year in invested_capital.index.year
            and year in net_income.index.year
        ):
    ...
    for symbol in tickers:
        ...
        try:
            ...
            result.update(
                {
                    "profile_ok": bool(
                        profile.get("name") or profile.get("finnhubIndustry")
                    ),
                    ...
                    "valuation_ok": bool(
                        np.isfinite(
                            dcf_intrinsic_from_base(
                                base_fcf, growth_base, shares, cash, debt
                            )
                        )
                    ),
...
pess_cut = st.sidebar.slider(
    "Recorte escenario pesimista (%)",
    10,
    90,
    50,
) / 100.0
...
if run_tests:
    if not finnhub_token or not alpha_key:
        st.error(
            "Necesitas API key de Finnhub y de Alpha Vantage para ejecutar el diagnóstico."
        )
    else:
...
if run:
    if not finnhub_token or not alpha_key:
        st.error(
            "Necesitas API keys de Finnhub y Alpha Vantage "
            "(FINNHUB_API_KEY y ALPHAV_API_KEY en secrets o barra lateral)."
        )
    else:
        try:
            ...
            growth_sales_pct = (
                fin["growth_sales"] * 100 if np.isfinite(fin["growth_sales"]) else None
            )
            per_teorico = (
                growth_sales_pct * 2 + 8 if growth_sales_pct is not None else None
            )
            ...
            mt_df = metrics_table_from_2020(fin)

            st.subheader(name)
            st.write(
                f"Industria Finnhub: {industry} | País: {country} | Bolsa: {exchange}"
            )
            st.caption(
                f"Peers detectados por Finnhub: {', '.join(peers) if peers else 'No disponibles'}"
            )

            c1, c2, c3, c4 = st.columns(4)
            c1.metric(
                "Precio actual",
                f"{price:.2f} {currency}" if np.isfinite(price) else "No disp.",
            )
            c2.metric(
                "Valor base",
                f"{v_base:.2f} {currency}" if np.isfinite(v_base) else "No disp.",
            )
            c3.metric(
                "Valor pesimista",
                f"{v_pes:.2f} {currency}" if np.isfinite(v_pes) else "No disp.",
            )
            c4.metric(
                "Valor personalizado",
                f"{v_cus:.2f} {currency}" if np.isfinite(v_cus) else "No disp.",
            )
            ...
            forward_pe_str = (
                f"{forward_pe:.2f}" if np.isfinite(forward_pe) else "No disponible"
            )
            if per_teorico is not None:
                st.caption(
                    f"Forward PER actual: {forward_pe_str} | "
                    f"PER teórico (2× crecimiento ventas + 8): {per_teorico:.2f}"
                )
            else:
                st.caption(
                    f"Forward PER actual: {forward_pe_str} | "
                    "PER teórico: No disponible"
                )
            ...
            if not mt_df.empty:
                ...
                st.caption(
                    "No hay datos suficientes para construir la tabla desde 2020."
                )

            st.subheader("Arquitectura de datos")
            st.markdown("- Perfil y peers: **Finnhub**.")
            st.markdown(
                "- Estados financieros históricos (income, balance, cash flow, EPS): "
                "**Alpha Vantage**."
            )
            st.markdown(
                "- Precios históricos para PER y valoraciones: "
                "**Yahoo Finance / yfinance** con caché."
            )
