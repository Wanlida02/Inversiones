...
st.title("Valoración de Acciones - Finnhub")
st.caption("Finnhub para perfil, peers y fundamentales; Yahoo Finance solo para precios históricos con caché.")

DB_PATH = "valuation_cache_finnhub.db"
...
def get_conn():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS company_profile (symbol TEXT PRIMARY KEY, raw_json TEXT, updated_at TEXT)")
    conn.execute("CREATE TABLE IF NOT EXISTS company_peers (symbol TEXT, peer TEXT, updated_at TEXT, PRIMARY KEY(symbol, peer))")
    conn.execute("CREATE TABLE IF NOT EXISTS company_financials (symbol TEXT, statement TEXT, freq TEXT, raw_json TEXT, updated_at TEXT, PRIMARY KEY(symbol, statement, freq))")
    return conn
...
def cache_profile(symbol, payload):
    conn = get_conn()
    conn.execute("INSERT OR REPLACE INTO company_profile(symbol, raw_json, updated_at) VALUES (?, ?, ?)", (symbol, json.dumps(payload), datetime.utcnow().isoformat()))
    conn.commit(); conn.close()


def load_profile(symbol):
    conn = get_conn()
    row = conn.execute('SELECT raw_json FROM company_profile WHERE symbol=?', (symbol,)).fetchone()
    conn.close()
...
def load_peers(symbol):
    conn = get_conn(); rows = conn.execute('SELECT peer FROM company_peers WHERE symbol=?', (symbol,)).fetchall(); conn.close()
...
def cache_financials(symbol, statement, freq, payload):
    conn = get_conn()
    conn.execute('INSERT OR REPLACE INTO company_financials(symbol, statement, freq, raw_json, updated_at) VALUES (?, ?, ?, ?, ?)', (symbol, statement, freq, json.dumps(payload), datetime.utcnow().isoformat()))
    conn.commit(); conn.close()


def load_financials(symbol, statement, freq):
    conn = get_conn()
    row = conn.execute('SELECT raw_json FROM company_financials WHERE symbol=? AND statement=? AND freq=?', (symbol, statement, freq)).fetchone()
    conn.close()
    return json.loads(row[0]) if row else None


def get_financials(symbol, statement, freq, token):
    cached = load_financials(symbol, statement, freq)
    if cached:
        return cached
    payload = finnhub_get('stock/financials', {'symbol': symbol, 'statement': statement, 'freq': freq}, token)
    cache_financials(symbol, statement, freq, payload)
    return payload
...
def series_from_financials(fin_payload, key):
    series = {}
    data = fin_payload.get('financials', []) if isinstance(fin_payload, dict) else []
    for row in data:
        period = row.get('period') or row.get('date') or row.get('year')
        metrics = row.get('metric', {})
        if period and isinstance(metrics, dict) and key in metrics:
            try:
                series[str(period)] = float(metrics[key])
            except Exception:
                continue
    if not series:
        return pd.Series(dtype=float)
...
def first_series(fin_payload, keys):
    for k in keys:
        s = series_from_financials(fin_payload, k)
        if len(s):
            return s
    return pd.Series(dtype=float)
...
def load_company_dataset(symbol, token):
    inc = get_financials(symbol, 'ic', 'annual', token)
    bs = get_financials(symbol, 'bs', 'annual', token)
    cf = get_financials(symbol, 'cf', 'annual', token)

    revenue = first_series(inc, ['revenue', 'totalRevenue'])
    ebitda = first_series(inc, ['ebitda'])
    net_income = first_series(inc, ['netIncome'])
    eps = first_series(inc, ['eps', 'epsBasicExclExtraItems', 'epsBasic', 'epsDiluted'])
    total_assets = first_series(bs, ['totalAssets'])
    total_liab = first_series(bs, ['totalLiabilities', 'totalLiabilitiesAndEquity'])
    shares = first_series(bs, ['shareIssued', 'commonStockSharesOutstanding'])
    ocf = first_series(cf, ['cashFlowFromOperations', 'netCashProvidedByUsedInOperatingActivities'])
    capex = first_series(cf, ['capitalExpenditures', 'paymentsToAcquirePropertyPlantAndEquipment'])
    ...
    return {
        ...
        'shares_series': ensure_datetime_index(shares),
...
def historical_pe_series(eps_series, price_hist, max_years=20):
    eps = ensure_datetime_index(eps_series)
...
def metrics_table_from_2000(fin, shares_fallback):
    revenue = fin['revenue']
    ebitda = fin['ebitda']
    net_income = fin['net_income']
    eps = fin['eps']
    fcf = fin['fcf']
    invested_capital = fin['invested_capital']
    shares_series = fin['shares_series']
    rows = []
    current_year = datetime.now().year
    for year in range(2000, current_year + 1):
        row = {'Year': year, 'Revenue_growth_%': np.nan, 'ROIC_%': np.nan, 'EBITDA_margin_%': np.nan, 'EPS': np.nan, 'Shares': np.nan, 'FCF': np.nan}
        if year in revenue.index.year:
            idx = np.where(revenue.index.year == year)[0]
            if len(idx):
                pos = idx[-1]
                if pos > 0:
                    prev = revenue.iloc[pos - 1]
                    cur = revenue.iloc[pos]
                    if pd.notna(prev) and prev != 0 and pd.notna(cur):
                        row['Revenue_growth_%'] = (cur / prev - 1) * 100
        ...
        if len(shares_series) and year in shares_series.index.year:
            row['Shares'] = float(shares_series[shares_series.index.year == year].iloc[-1])
        elif np.isfinite(shares_fallback):
            row['Shares'] = float(shares_fallback)
        if year in fcf.index.year:
            row['FCF'] = float(fcf[fcf.index.year == year].iloc[-1])
        rows.append(row)
    df = pd.DataFrame(rows).set_index('Year')
    non_empty = df.dropna(how='all')
    return non_empty if not non_empty.empty else df
...
def run_diagnostics(token, tickers=TEST_TICKERS):
    ...
    for symbol in tickers:
        ...
        try:
            ...
            fin = load_company_dataset(symbol, token)
            shares = info.get('sharesOutstanding', np.nan)
            cash = info.get('totalCash', 0.0)
            debt = info.get('totalDebt', 0.0)
            pe_hist = historical_pe_series(fin['eps'], hist, max_years=20)
            base_path = intrinsic_path(fin['base_fcf'], fin['growth_base'], shares, cash, debt)
            result.update({
                ...
                'valuation_ok': bool(np.isfinite(dcf_intrinsic_from_base(fin['base_fcf'], fin['growth_base'], shares, cash, debt))),
...
per_years = st.sidebar.slider('Años de PER histórico', 5, 25, 15)
...
if run:
    if not finnhub_token:
        st.error('Necesitas API key de Finnhub (FINNHUB_API_KEY en secrets o barra lateral).')
    else:
        try:
            profile = get_profile(ticker, finnhub_token)
            peers = get_peers(ticker, finnhub_token)
            info, hist = yahoo_price_history(ticker)
            fin = load_company_dataset(ticker, finnhub_token)
            shares = info.get('sharesOutstanding', np.nan)
            cash = info.get('totalCash', 0.0)
            debt = info.get('totalDebt', 0.0)
            ...
            g_cus = custom_growth
            v_base = dcf_intrinsic_from_base(fin['base_fcf'], g_base, shares, cash, debt)
            v_pes = dcf_intrinsic_from_base(fin['base_fcf'], g_pes, shares, cash, debt)
            v_cus = dcf_intrinsic_from_base(fin['base_fcf'], g_cus, shares, cash, debt)
            ...
            base_path = intrinsic_path(fin['base_fcf'], g_base, shares, cash, debt)
            pes_path = intrinsic_path(fin['base_fcf'], g_pes, shares, cash, debt)
            cus_path = intrinsic_path(fin['base_fcf'], g_cus, shares, cash, debt)
            mt_df = metrics_table_from_2000(fin, shares)
            ...
            st.markdown(f"[Ver PER histórico en Macrotrends](https://www.macrotrends.net/stocks/charts/{ticker}/{ticker.lower()}/pe-ratio)  ")
            ...
            st.subheader('Fundamentales anuales desde 2000')
            if not mt_df.empty:
                ...
                st.caption('No hay datos suficientes para construir la tabla desde 2000.')
            st.markdown(f"[Ver crecimiento de ventas en CompaniesMarketCap](https://companiesmarketcap.com/{ticker.lower()}/revenue/)  ")

            st.subheader('Arquitectura de datos')
            st.markdown('- Perfil, peers y fundamentales anuales: **Finnhub**.')
